...
local function MenuMovement(tomato)
    ...
    flux.to(tomato, 2, { x = startX + 50})
        :after(2, {y = startY + 50 })
        :after(2, { x = startX - 50})
        :after(2, {y = startY - 50 })
        :oncomplete(function()
            -- Once the final step finishes, call this function again to loop!
            MenuMovement(tomato) 
        end)

end

local function MenuSpawner(dt)
    if currentScene ~= 1 then return end

    if MenuSpawnInterval >= 0 then
        MenuSpawnInterval = MenuSpawnInterval - dt

    elseif MenuSpawnInterval < 0 then --test
        local RandInt = love.math.random(1, 4)
        local newTomato = Tomato.new(TomatoImages[RandInt], (RandInt..#Tomatoes..ConstantSpawnInterval), (VIRTUAL_WIDTH/2 + love.math.random(-50, 50)), (VIRTUAL_HEIGHT/2+ love.math.random(-50, 50)))
        table.insert(MenuTomatoes, newTomato) --maybe change to dynamic checking of coords
        MenuMovement(newTomato)
        MenuSpawnInterval = 20 --test value
